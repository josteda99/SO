...
char *builtin_str[] = {
    "cd",   // cada uno es puntero
    "help",
    "exit"};

int (*builtin_func[])(char **) = { // directorio a cada string del comado mapea la
    &lsh_cd,                       // funcion correspondiente
    &lsh_help,          
    &lsh_exit};

int lsh_num_builtins()
{
   return sizeof(builtin_str) / sizeof(char *); // tamaño del builtin_str / tam dato char*
   // retorna 3 
}

/* Built-in function implementations */
int lsh_cd(char **args) // funcionamiento del comando cd
{
   if (args[1] == NULL)
   { // revisa si es correcto y no solo puso cd
...
int lsh_help(char **args)
{
   ...
   { //imprimiendo todas las comandos
      printf(" %s\n", builtin_str[i]);
   }
...
{
   pid_t pid, wpid; // crea variables de pid(process ID)
   int status;      // saber si el padre espera o no

   pid = fork(); // crea un prcoeso hijo
   if (pid == 0)
   {  /* child process */
      //se creo bien
      if (execvp(args[0], args) == -1)
         perror("lsh");   // si puso mal imprime el error
      exit(EXIT_FAILURE); // salgase
   }
   else if (pid > 0)
   { /* parent process */
      if (!conc)//concurrente
   ...
   conc = false; // lo pone false para que no lo ejecute concurrente
   return 1;
}
...
char **lsh_split_line(char *line)
{
   ...
   int position = 0;
   char **tokens = malloc(cur_bufsize * sizeof(char *)); //crea la posicion de memoria para los comandos
   char *token;
   ...
   while (token != NULL)
   {
      ...
      if (position >= cur_bufsize)//entra si no alcanza
      { //
         cur_bufsize += LSH_TOK_BUFSIZE;// da mas espacio en memoria
         tokens = realloc(tokens, cur_bufsize * sizeof(char *)); // mira si si necesitaba memoria, reacomoda 
                        // puntero, nuevo tamaño
         if (!tokens)// por sihubo error, si los tokens apuntan a la nada
         {
            fprintf(stderr, "lsh: allocation error\n");
            exit(EXIT_FAILURE);
         }
      ...
      token = strtok(NULL, LSH_TOK_DELIM); // rompe en tokens
   }
   
   if (position > 0 && strcmp(tokens[position - 1], "&") == 0) // hay un token && comapra los string
   {
      conc = true;
...
{
   if (cur_pos == -1 || history[cur_pos] == NULL)// no hay nada en el hisotrial
   {
   ...
   { // revisar los comandos
      ...
      if (cur_pos != LSH_HIST_SIZE && history[cur_pos + 1] != NULL)// si la posicion no esta ocupada aun
      {
         last_pos = cur_pos + 1;// le asigna +1 de memoria
      }
   ...
   else
   {
      ...
      if (strcmp(args[0], "!!") == 0)
      {
         ...
         strcat(command, history[cur_pos]);// al comand le apend history
         printf("%s\n", command);// imprime
         cmd_args = lsh_split_line(command);// splitea
         int i;
         for (i = 0; i < lsh_num_builtins(); i++)
         {
            if (strcmp(cmd_args[0], builtin_str[i]) == 0)
            {
               return (*builtin_func[i])(cmd_args); // retorna la direccion de la funcion
            }
         }
      ...
      else if (args[0][0] == '!')
      {
         ...
         command = malloc(sizeof(history[cur_pos]));
         strcat(command, history[offset]);// comand concatenan hisorial
         ...
         return lsh_launch(cmd_args);// lanza si no reconoce el comando en el diccioanrio
      }
      else
      {
         perror("lsh");
...
int lsh_execute(char *line)
{
   ...
   // suma una posicion mas al historial
   ...
   {
      strcat(history[cur_pos], *temp_args); // comando del hisorial + temp_args
      strcat(history[cur_pos], " ");// comando del historial + " "
      temp_args++;
   }
   //printf("%s\n", history[cur_pos]);
   //history[count] = '\0';
...
char *lsh_read_line(void)
{
   ...
   if (!buffer)// si no se pudo reservar memoria
   {
      fprintf(stderr, "lsh: allocation error\n");
      exit(EXIT_FAILURE);
   }
   ...
   while (1)
   {
      ...
      if (position >= cur_bufsize)// si falta espacio pues le da mas espacio 
      {
         cur_bufsize += LSH_RL_BUFSIZE;
...
void lsh_loop(void)
{
   ...
   do
   {
      ...
      free(line);// liberacion de punteros
   } while (status);
}
